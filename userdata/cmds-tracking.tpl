...
Replace <customer_secret_access_key>, <customer_secret_secret_key>, <mysql_mlflow_username> and <mysql_mlflow_password>. We don't save these values in this file.

sudo docker run -d --network="host" --rm -p 5000:5000 --name mlflow-tracking -e MLFLOW_S3_ENDPOINT_URL=${s3_endpoint_url} -e AWS_ACCESS_KEY_ID=<customer_secret_access_key> -e AWS_SECRET_ACCESS_KEY=<customer_secret_secret_key> -e BUCKET_URL=${bucket-url} -e MYSQL_USER=<mysql_mlflow_username> -e MYSQL_PWD='<mysql_mlflow_password>' -e MYSQL_HOST=${database_ip} -e MYSQL_PORT=${database_port} -e DB_NAME=${dbname} ${region_registry}.ocir.io/${tenancy_namespace}/${repo_name}/mlflow-tracking:0.0.1 
